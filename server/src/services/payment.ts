...
export async function createPayment(input: CreatePaymentInput): Promise<CreatePaymentResult> {
  ...
  try {
    const payment =
      input.mode === "ITEMS" ? await createItemBasedPayment(input, method) : await createAmountBasedPayment(input, method);
...
export async function voidPayment(paymentId: string, staffId: string, reason: string): Promise<CreatePaymentResult> {
  ...
  const idempotencyKey = `${kind.toLowerCase()}:${original.id}`;
