...
const envSchema = z.object({
  ...
  /**
   * 쿠폰 번호 조회의 직원(비로그인 시 IP)당 5분 허용 횟수(요구사항.md §8).
   *
   * 번호가 3자리(001~999)라 인증된 직원이라도 무한히 훑을 수 있으면 안 된다. 조회는 상태를
   * 바꾸지 않지만 "어떤 번호가 살아있는지"를 알려주므로, 정상 업무(한 거래에 한두 번)에는
   * 걸리지 않고 전수조사는 막는 수준으로 잡는다. 실패한 조회는 감사 로그에 남는다.
   */
  COUPON_LOOKUP_RATE_LIMIT_PER_5MIN: z.coerce.number().int().min(5).default(60),
