...
**이번 작업 목적:** HANDOFF.md에 마지막으로 남아있던 "모바일 뷰포트 Playwright" 항목 처리.

**구현 내용:** 별도 모바일 `projects`를 `playwright.config.ts`에 추가해 기존 3개 스펙을 전부 두 번(데스크톱+모바일) 돌리는 대신, **손님이 보는 `/t/:slug` 화면을 여는 컨텍스트만** `devices["iPhone 13"]`로 띄우도록 변경(`scenario-a.spec.ts`의 손님 컨텍스트, `security-and-edge-cases.spec.ts`의 시나리오 I/D 컨텍스트). FRONT/POS/SERVING/ADMIN은 실제로 매장 데스크톱·태블릿에서 쓰는 화면이라 데스크톱 그대로 유지 — 손님 화면만 실제 사용 환경(휴대폰)과 일치시키는 게 실행 시간 대비 실효성이 더 높다고 판단.

**실행한 테스트:** `npm run test:e2e` 로컬 2회 연속 4/4 통과.

**남은 문제:** 실기기 테스트만 남음(사람이 직접 확인 필요). HANDOFF.md의 "아직 안 된 것" 항목은 사실상 이것 하나.
...
**이번 작업 목적:** 이전 커밋(UI 폴리싱)이 CI에서 실패해 원인 조사 및 수정. 이어서 ADMIN 나머지 탭(메뉴/사용자/이용권/감사로그/결제내역)도 스크린샷으로 마저 검토.

**CI 실패 원인 및 수정 (`e2e/tests/security-and-edge-cases.spec.ts`, 내 이전 커밋과는 무관한 기존 결함 2건):**
1. **시나리오 H 플레이키**: `useStaffMe`가 인가 실패 시 `window.location.href`로 하드 리다이렉트하는데, 그 리다이렉트가 `/admin` 최초 페이지의 `load` 이벤트보다 먼저 발생하면 그 내비게이션 자체가 `net::ERR_ABORTED`로 취소되어 `page.goto("/admin")` 이후의 `waitForURL`이 "frame was detached" 오류를 던진다. `page.goto("/admin").catch(() => undefined)`로 완화 — Playwright 공식 문서에도 나오는 "빠른 연속 리다이렉트" 표준 대응.
2. **시나리오 D 플레이키**: `CartSheet`의 "주문하기" 버튼은 첫 클릭에서 즉시(동기) `disabled` 처리되고, 주문 성공 시 시트 자체가 DOM에서 사라진다. `Promise.all([클릭, 클릭])`으로 두 번째 클릭이 "비활성화된 엘리먼트" 또는 "DOM에서 제거된 엘리먼트"를 계속 재시도하다 액션 타임아웃으로 실패 → 테스트 전체 실패. 두 클릭 모두에 `{ timeout: 2000 }` + `.catch(() => undefined)`를 적용해, 클라이언트 방어로 인해 두 번째 클릭이 그냥 씹히는 경우를 정상으로 취급하도록 수정. 로컬에서 2회 연속 전체 통과 확인.

**ADMIN 나머지 탭 검토 결과:** 메뉴/사용자/이용권/감사로그 탭은 특별한 문제 없음. **결제내역 탭만 다른 탭과 다르게 열자마자 아무것도 안 보이고 "검색" 버튼을 눌러야 데이터가 뜨는 비일관성**을 발견 — `useEffect`로 마운트 시 필터 없이 즉시 검색을 1회 실행하도록 수정(`client/src/pages/admin/PaymentsPanel.tsx`). 이제 테이블/메뉴/사용자/이용권/결제내역 전부 탭을 열면 즉시 최신 목록이 보인다.

**겪은 오탐**: 이번에도 Vite dev 서버가 파일 저장을 즉시 반영하지 못해(재시작 전) 수정이 반영 안 된 것처럼 보였다가, 완전 재시작 후 정상 확인됨. `curl`로 소스를 확인할 때 한국어 주석 텍스트로 grep하는 건 신뢰할 수 없다는 것도 확인(Vite/esbuild가 dev 번들에서도 주석을 제거함) — 앞으로는 실제 동작(계산된 스타일, DOM 구조, 렌더링된 텍스트)으로만 반영 여부를 판단할 것.

**실행한 테스트:** `npx tsc -p client/tsconfig.json --noEmit` 클린. `npm run test:e2e` 로컬 2회 연속 4/4 통과.

**다음 작업자가 가장 먼저 할 일:** 이 커밋의 CI 결과를 `gh run view`로 반드시 확인한 뒤 이어서 모바일 뷰포트 Playwright 프로젝트 추가로 진행할 것.
