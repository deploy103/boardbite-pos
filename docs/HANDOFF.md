...
Phase 2~6 구현 완료. Phase 7(안정화)도 사실상 완료 — 감사로그 정리, 운영 매뉴얼, Playwright E2E(모바일 프로필 포함), UI 디자인 폴리싱 1차 패스까지 끝났다. 이 환경에서 할 수 있는 검증은 다 했고, **남은 유일한 항목은 사람이 직접 해야 하는 실기기 테스트뿐**. (2026-09-12)
...
- **Phase 7 안정화 (완료)**:
  ...
  - **Playwright E2E 도입**(`e2e/` 워크스페이스): 시나리오 A(정상 흐름 전체, 4개 역할의 실제 브라우저 컨텍스트), 시나리오 D(더블탭), H(권한 우회+클라이언트 리다이렉트), I(CLOSED 테이블 차단) — 전부 통과. CI에도 `e2e` 잡으로 추가됨. 손님 대면 컨텍스트(A/D/I)는 `devices["iPhone 13"]`로 실행해 실제 사용 환경(휴대폰)과 맞춤.
  - **UI 디자인 폴리싱 1차 패스**(브라우저를 직접 볼 수 없는 환경이라 Playwright로 로그인→조작→스크린샷→`Read` 도구로 검토하는 방식 사용, FRONT/손님/POS/SERVING/ADMIN 전 탭 확인 완료): KDS 4단 보드가 표준 데스크톱 폭에서 4번째 컬럼이 스크롤 없이 잘리던 문제, 운영설정 화면의 숫자 입력칸이 placeholder만 있어 값이 채워지면 라벨이 사라지던 문제, 결제수단 화면 버튼이 좁은 flex 행에서 줄바꿈되던 문제, 결제내역 탭만 다른 탭과 달리 열자마자 비어있던 비일관성 — 총 4건 수정.
  - 이 과정에서 실제 버그 2건 + WSL 환경 오탐 다수 발견/정리: 백업 UI 누락, `fs.Stats.birthtime` 미지원으로 백업 생성일자가 1970년으로 나오는 버그. **WSL dev 서버가 파일 변경을 조용히 반영하지 못하는 현상이 반복 재현**되어 대응 요령을 아래 "인프라 주의사항"에 강화해 기록함.
- **테스트 총계**: 서버 13개 파일/90개(Vitest+Supertest), E2E 2개 파일/4개(Playwright, 손님 컨텍스트는 모바일 프로필). 서버·클라이언트 `tsc --noEmit` 클린, `npm run build`(client) 성공, CI 3개 잡(secret-scan/test/e2e) 전부 그린.

## 아직 안 된 것

- **실기기 테스트**: iPhone Safari/Android Chrome/iPad에서의 실제 확인은 이 환경에서 수행 불가 — 사람이 직접 해야 함. `docs/TEST-PLAN.md` §5 체크리스트 참고. 이것만 끝나면 Phase 7이 완전히 마무리된다.
- 매출 리포트 `since` 필터는 시작일만 지원(종료일 없음) — 요구사항에 없던 선택적 개선 사항.
...
1. `docs/DEVLOG.md` 최신 항목을 읽는다. dev 서버가 죽어있으면 재시작한다(아래 "인프라 주의사항" #1 참고 — 살아있어도 stale일 수 있다).
2. 사람이 실기기(iPhone/Android/iPad)로 직접 접속해 확인 — 이 문서와 `docs/TEST-PLAN.md` §5 체크리스트를 따른다. 이게 끝나면 이 프로젝트에 남은 계획된 작업이 없다.
3. 그 외에는 사용자가 새로 요청하는 기능/버그 수정을 진행하거나, "아직 안 된 것"의 선택적 개선 사항(매출 리포트 종료일 필터, `npm audit` 취약점 업그레이드 검토) 중 우선순위가 있는 것을 판단해서 진행한다.
